...
def exercice(enonce: str, chapter_name: str, lesson_pdf:str, lesson_name:str, nom_exo) -> None:
    ...
    execute_button = tk.Button(root, text="Exécuter", command=execute_code)
    execute_button.pack()
    ...
    enonce_txt = Label(text=f"{enonce}", bg="Black", fg="White", font=("Arial", 20))
    enonce_txt.pack(side=LEFT)

    #Bouton pour accéder à la correction
    correction_btn = Button(root, text="Accéder à la correction", command=lambda:open_given_file(get_correction(chapter_name, lesson_name, nom_exo)))
    if nb_indices_depenses >=3:
        widget_lst.append(correction_btn)
        correction_btn.pack(side=RIGHT)
